...
(defn- genre-by-name [m4a]
  (try
    (let [box (Path/getPath m4a "moov/udta/meta/ilst/©gen")]
      (.getValue box))
    (catch Exception _
      nil)))

(defn- genre-by-id [m4a]
  (try
    (let [box (Path/getPath m4a "moov/udta/meta/ilst/gnre")
          genre-id (.getValue box)]
      (get id3v1-genres (- genre-id 1)))
    (catch Exception _
      nil)))

(defn- genre [m4a]
  (or (genre-by-name m4a) (genre-by-id m4a)))
...
(comment
  ...
  (.getValue (Path/getPath m4a "moov/udta/meta/ilst/©gen"))
